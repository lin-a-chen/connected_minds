		...
		if (emailToken) {
			...
			if (result.success) {
				...
				return new Response(
					JSON.stringify({
						success: false,
						data: "User with this token wasn't found",
					}),
					{ status: 500 }
				);
			}
			...
			if (result.success) {
				...
				return new Response(
					JSON.stringify({ success: false, data: "No users found" }),
					{ status: 500 }
				);
			}
		}
...
export async function DELETE(req) {
	try {
		...
		if (result.success) {
			...
			return new Response(
				JSON.stringify({ success: false, data: "User wasn't deleted" }),
				{ status: 500 }
			);
		}
	} catch (error) {
...
export async function PUT(req) {
	...
	try {
		...
		const result = await User.updateById(
			...
			body.email,
			body.phone_number,
			findUserResult.data.password,
			body.is_activated,
			findUserResult.data.email_token,
			findUserResult.data.email_token_expires_at
		);
		if (!result.success) {
			return new Response(
				JSON.stringify({
		...
		if (!body.password && !body.oldPassword) {
			...
			if (!findUser.success) {
				return new Response(
					JSON.stringify({
						success: false,
						data: "Не вдалося знайти користувача",
					}),
					{ status: 500 }
				);
			}
			...
			if (!isPassCorrect) {
				return new Response(
					JSON.stringify({ success: false, data: "Пароль невірний" }),
					{ status: 405 }
				);
			}
			...
			if (result.success) {
				...
				return new Response(
					JSON.stringify({
						success: false,
						data: "Не вдалося оновити пароль",
					}),
					{ status: 500 }
				);
			}
