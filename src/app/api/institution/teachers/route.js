	...
	try {
		...
		if (userDeleteResult.success) {
			return new Response(
				JSON.stringify({
					success: true,
					data: "Teacher was successfully deleted",
				}),
				{ status: 200 }
			);
		} else {
	...
	try {
		...
		if (!teacherResult.data) {
			return new Response(
				JSON.stringify({
					success: false,
					data: "Вчительку/вчителя не знайдено або такий акаунт не існує",
				}),
				{ status: 500 }
			);
		}
		...
		const teacherUpdateResult = await Teacher.updateById(
		...
		if (!teacherUpdateResult.success) {
			return new Response(
				JSON.stringify({
					success: false,
					data: "Не вдалося оновити дані вчительки/вчителя",
				}),
				{ status: 500 }
			);
		}
	...
	try {
		...
		if (!teacherInsertResult.success) {
			return new Response(
				JSON.stringify({
					success: false,
					data: "Не вдалося зареєструвати вчительку/вчителя",
				}),
				{ status: 500 }
			);
		}
		...
		if (!roleAssignmentResult.success) {
			const data = JSON.stringify({
				success: false,
				data: "Помилка реєстрації вчительки/вчителя",
			});
