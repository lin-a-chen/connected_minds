...
export const POST = async (req) => {
    ...
    try{
        ...
        if (!userInsertResult.success){
            const data = JSON.stringify({success: false, data: 'Помилка реєстрації користувача'});
            return new Response(data, {status: 500});
        }

        const roleAssignmentResult = await Role.assignRoleByUserId(userInsertResult.data, "INSTITUTION_ADMIN", body.useedCode);
        if (!roleAssignmentResult.success){
            const data = JSON.stringify({success: false, data: 'Помилка призначення користувача адміністратором'});
            return new Response(data, {status: 500});
        }
