...
import { TbCalendar, TbPencilMinus, TbBuildingEstate } from "react-icons/tb";
...
export default function AddSchoolchild({ onClose, isVisible }) {
	...
	const onSubmit = async (data) => {
		const validateResponse = await fetch(
			`/api/institution/schoolchildren/validate`,
			{
				method: "POST",
				headers: {
								...
								<div>
									<input
										...
										{...register("classNumber", {
											required:
												"Номер класу не може бути пустим",
											min: {
												value: 1,
												message:
													"Не буває класів менше ніж 1",
											},
											max: {
												value: 12,
												message:
													"Не буває класів більше ніж 12",
											},
										})}
									/>
									<select
										...
										{...register("classLetter", {
											required:
												"Літера класу не може бути пустою",
										})}>
										<option value={""}>
											Оберіть літеру класу
										</option>
										{classLetters &&
