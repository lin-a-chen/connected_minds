...
class AcademicRecord {
	...
	static async add(grade, present, studentId, teacherId, classId, subjectId, date) {
		const connection = await connectToAppDatabase();
		try {
			const result = await connection.query(`INSERT INTO academic_records(grade, present, student_id, teacher_id, class_id, subject_id, date) VALUES(?, ?, ?, ?, ?, ?, ?)`, [grade, present, studentId, teacherId, classId, subjectId, date]);
			await connection.end();
