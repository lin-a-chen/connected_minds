...
export default function InstitutionAdminClasses() {
    ...
    const handleDelete = async (el) => {
        const response = await fetch(`/api/institution/classes?id=${el.id}`, {method: 'DELETE'});
        const result = await response.json();
        if (!result.success){
            toast.error(result.data);
            return;
	...
	return (
        ...
        <button onClick={handleAddClass} className={styles.addClassButton}><LuCross/>Додати клас</button>
        {addClassModuleOpened && <AddClass isVisible={addClassModuleOpened} onClose={handleOnCloseClassModule}/>}
        <div className={styles.classesContainer}>
            <div className={styles.column0}>{classes.map((el, index) => (
                    index % 2 !== 0 && <div
					key={el.id}
				  ...
				  {console.log('class', el)}
					...
					<div
						className={`${styles.members} ${
							expandedClasses[el.id] ? styles.membersExpanded : ""
						}`}
						onClick={expandedClasses[el.id] ? null : () => toggleExpandedClass(el.id)}>
						<button className={styles.addMemberButton}>
							<FaPlus />
						</button>
						<div
							className={`${styles.membersCircles} ${
								expandedClasses[el.id] ? styles.expanded : ""
							}`}>
							{schoolchildren[el.id]?.length > 0 ? (
								schoolchildren[el.id]
									...
									.map((child, index) => (
										<div 
                                            ...
                                            onClick={() => window.location.href = `/institution/schoolchild/${child.id}`}
                                            >
					...
					{expandedClasses[el.id] && (
						<button
							...
							onClick={() => toggleExpandedClass(el.id)}>
							Hide
						</button>
					)}
				</div>			
			))}</div>
            <div className={styles.column1}>{classes.map((el, index) => (
                    index % 2 === 0 && <div
					key={el.id}
					className={`${styles.class}`}
					onDrop={(e) => onDrop(e, el.id)}
